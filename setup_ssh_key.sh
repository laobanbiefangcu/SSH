...
可以组合 -u/-k/-f 与 -K/-P，一次完成密钥生成和登录策略切换。
USAGE
}
...
reload_sshd() {
  if command -v systemctl >/dev/null 2>&1; then
    ...
    service ssh reload 2>/dev/null || service ssh restart 2>/dev/null || service sshd restart 2>/dev/null
  fi
}
...
main() {
  ...
  while getopts ":u:k:fhKPm" opt; do
    case "$opt" in
      u) username=$OPTARG ;;
      k) key_path=$OPTARG ;;
      f) force_recreate=1 ;;
  ...
  if [[ -z $username && -z $desired_mode ]]; then
